 ...
 * @author 이경륜
 */
public class DetailPhotoHandler implements CommandHandler{
	
	private static final String VIEW_PAGE = "/views/photo/detailPhoto.jsp";
	...
	private String getJspPage(HttpServletRequest req, HttpServletResponse resp, String memId) {
		...
		// 글NO, 작성자명, 카테고리5개, 해시태그, 조회수, 작성일, 좋아요수, 좋아요여부, 작성자프사경로, 작성자프사저장명, 팔로우여부, 댓글수
		PhotoDetailVO photoDetailVO = postService.selectPhotoDetail(paramVO);
		req.setAttribute("photoDetailVO", photoDetailVO);
		
		// 2. 파일 관련: 파일ID, 글NO, 사진, 내용, 저장경로, 저장이름 (List<AtchFileVO>) -- 파일패키지
		// 사진 첨부파일 및 글 내용(atch_file) 로드 후 req객체에 셋팅
		List<AtchFileVO> photoFileList = photoService.selectPhotoFiles(postNo);
		req.setAttribute("photoFileList", photoFileList);
		...
		// 4. 멤버의 최근게시글 4장 (List<MypagePhotoVO>) -- photo 패키지
		List<MypagePhotoVO> fourPhotoList = photoService.selectFourPhotos(postNo);
		req.setAttribute("fourPhotoList", fourPhotoList);

		// 5. 로그인id의 프로필 이미지 (댓글 작성칸)
